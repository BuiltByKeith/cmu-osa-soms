var FileUploadWithPreview = (function (Q) {
    "use strict";
    const b = "";
    var e = ((i) => (
        (i.IMAGE_ADDED = "fileUploadWithPreview:imagesAdded"),
        (i.IMAGE_DELETED = "fileUploadWithPreview:imageDeleted"),
        (i.CLEAR_BUTTON_CLICKED = "fileUploadWithPreview:clearButtonClicked"),
        (i.IMAGE_MULTI_ITEM_CLICKED =
    ...
    class k {
        constructor(A, g = {}) {
            ...
            const {
                ...
                multiple: B,
                presetFiles: I,
                showDeleteButtonOnImages: t,
            } = g;
            (this.options.showDeleteButtonOnImages = t ?? !0),
                (this.options.maxFileCount = C ?? 0),
                (this.options.presetFiles = I ?? []),
                (this.options.multiple = B ?? !1),
                (this.options.accept = E ?? this.options.accept);
            const {
                browse: s,
                chooseFile: f,
                label: y,
            ...
            (this.options.text.chooseFile = f ?? this.options.text.chooseFile),
                (this.options.text.browse = s ?? this.options.text.browse),
                (this.options.text.label = y ?? d),
                (this.options.text.selectedCount =
                    D ?? this.options.text.selectedCount);
    ...
    `);
            const u = this.el.querySelector(
                    ".custom-file-container .input-hidden"
                ),
                m = this.el.querySelector(
                    ".custom-file-container .input-visible"
                ),
                h = this.el.querySelector(
                    ".custom-file-container .image-preview"
                ),
                J = this.el.querySelector(
                    ".custom-file-container .clear-button"
                );
            if (u != null && m != null && h != null && J != null)
                (this.inputHidden = u),
                    (this.inputVisible = m),
                    (this.inputVisible.innerHTML =
            ...
            (this.options.images.baseImage =
                P ?? this.options.images.baseImage),
                (this.options.images.successPdfImage =
                    L ?? this.options.images.successPdfImage),
                (this.options.images.successVideoImage =
                    G ?? this.options.images.successVideoImage),
                (this.options.images.successFileAltImage =
                    U ?? this.options.images.successFileAltImage),
        ...
        bindClickEvents() {
            this.inputHidden.addEventListener(
                ...
                (A) => {
                    const g = A.target,
                        { files: E } = g;
                    E != null && (this.addFiles(E), (g.value = ""));
                },
                !0
            ),
                this.clearButton.addEventListener(
                    ...
                    () => {
                        const A = { detail: { uploadId: this.uploadId } },
                            g = new CustomEvent(e.CLEAR_BUTTON_CLICKED, A);
                ...
                this.imagePreview.addEventListener("click", (A) => {
                    ...
                    if (g) {
                        if (
                            ...
                            const E = g.getAttribute("data-upload-name"),
                                C = this.cachedFileArray.findIndex(
                                    ({ name: B }) => B === E
                                );
                            this.deleteFileAtIndex(C);
                        }
                        if (
                            g.matches(
                            ...
                            const E = g.querySelector(
                                ...
                                C =
                                    E == null
                                        ? void 0
                                        : E.getAttribute("data-upload-name"),
                                B = this.cachedFileArray.findIndex(
                                    ({ name: s }) => s === C
                                );
                            if (B < 0) return;
                            const I = {
                                    detail: {
                                        ...
                                        file: this.cachedFileArray[B],
                                        index: B,
                                        uploadId: this.uploadId,
                                    },
                                },
                                t = new CustomEvent(
                                    e.IMAGE_MULTI_ITEM_CLICKED,
                                    I
                                );
        ...
        async addImagesFromPath(A) {
            A.forEach(async (g) => {
                try {
                    const E = "image/jpeg",
                        B = await (await fetch(g, { mode: "cors" })).blob(),
                        I = new File([B], "preset-file", { type: B.type || E });
                    this.addFiles([I]);
                } catch (E) {
                    E instanceof Error &&
                        console.warn(`${E.message.toString()}`),
                        console.warn(
                            "Image cannot be added to the cachedFileArray."
                        );
                }
            });
        }
        addFiles(A) {
            ...
            const E = {
                ...
                C = new CustomEvent(e.IMAGE_ADDED, E);
        ...
        addFileToPreviewPanel(A) {
            this.cachedFileArray.length === 0
                ...
                ? (this.inputVisible.textContent = A.name.split(c)[0])
            ...
            g.readAsDataURL(A),
                (g.onload = () => {
                    if (!this.options.multiple) {
                        ...
                        A.type.match("image/webp") ||
                        A.type.match("image/gif")
                            ? (B = `url("${g.result}")`)
                            : A.type.match("application/pdf")
                            ? (B = `url("${this.options.images.successPdfImage}")`)
                            : A.type.match("video/*") &&
                              (B = `url("${this.options.images.successVideoImage}")`),
                            (this.imagePreview.style.backgroundImage = B);
                        return;
                    }
                    this.imagePreview.style.backgroundImage = `url("${this.options.images.backgroundImage}")`;
                    const E = (B) => `
        <span class="image-preview-item-clear">
          <span class="image-preview-item-clear-icon" data-upload-name="${B}">
            &times;
          </span>
        </span>
      `;
                    let C = this.options.images.successFileAltImage;
                    A.type.match("image/png") ||
                    A.type.match("image/jpeg") ||
                    A.type.match("image/webp") ||
                    A.type.match("image/gif")
                        ? (C = g.result)
                        : A.type.match("application/pdf")
                        ? (C = this.options.images.successPdfImage)
                        : A.type.match("video/*") &&
                          (C = this.options.images.successVideoImage),
      ...
      `);
        ...
        deleteFileAtIndex(A) {
            ...
            const g = {
                ...
                E = new CustomEvent(e.IMAGE_DELETED, g);
            window.dispatchEvent(E);
        }
        refreshPreviewPanel() {
            const g = this.imagePreview.querySelectorAll(".image-preview-item");
            Array.from(g).forEach((C) => C.classList.add(R)),
                setTimeout(() => {
                    if (
                        ((this.imagePreview.innerHTML = ""),
                        !this.cachedFileArray.length)
                    ...
                    this.cachedFileArray.forEach((C) =>
                        this.addFileToPreviewPanel(C)
                    );
                }, 200);
        }
        addBrowseButton(A) {
    ...
    return (
        (Q.DEFAULT_BACKGROUND_IMAGE = l),
        (Q.DEFAULT_BASE_IMAGE = a),
        (Q.DEFAULT_SUCCESS_FILE_ALT_IMAGE = r),
        (Q.DEFAULT_SUCCESS_PDF_IMAGE = o),
        (Q.DEFAULT_SUCCESS_VIDEO_IMAGE = n),
        (Q.Events = e),
        (Q.FileUploadWithPreview = k),
